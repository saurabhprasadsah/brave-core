...
NativeAdsClient::NativeAdsClient(
    id<NativeAdsClientBridge> bridge)
    : bridge_(bridge) {}
...
void NativeAdsClient::GetClientInfo(
...
void NativeAdsClient::SetIdleThreshold(
    const int threshold) {
...
void NativeAdsClient::LoadUserModelForLanguage(
...
void NativeAdsClient::CloseNotification(
    const std::string& id) {
...
void NativeAdsClient::ConfirmAd(
    std::unique_ptr<ads::NotificationInfo> info) {
  [bridge_ confirmAd:std::move(info)];
}

void NativeAdsClient::ConfirmAction(
    const std::string& uuid,
    const std::string& creative_set_id,
    const ads::ConfirmationType& type) {
  [bridge_ confirmAction:uuid creativeSetId:creative_set_id
      confirmationType:type];
}

uint32_t NativeAdsClient::SetTimer(
    const uint64_t time_offset) {
  return [bridge_ setTimer:time_offset];
}

void NativeAdsClient::KillTimer(
    uint32_t timer_id) {
  [bridge_ killTimer:timer_id];
}

void NativeAdsClient::URLRequest(
  ...
  [bridge_ URLRequest:url headers:headers content:content
      contentType:content_type method:method callback:callback];
}

void NativeAdsClient::Save(

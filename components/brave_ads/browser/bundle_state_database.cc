...
bool BundleStateDatabase::GetCreativeAdNotifications(
  ...
  info_sql.BindString(0, base::JoinString(categories, ", ").c_str());
...
bool BundleStateDatabase::GetCreativePublisherAds(
  ...
  sql::Statement info_sql(
      db_.GetUniqueStatement(
          ...
          "datetime('now','localtime'));"));
  info_sql.BindString(0, base::JoinString(categories, ", ").c_str());
  info_sql.BindString(1, base::JoinString(sizes, ", ").c_str());
  // TODO(tmancey): Parse host and remove subdomain
  const std::string host = GURL(url).host();
  info_sql.BindString(2, host);
